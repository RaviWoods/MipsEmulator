...
struct test_instr {
	...
	test_instr(uint32_t pc_diff, string name_in,  uint32_t src1_in, uint32_t src1_data_in, uint32_t src2_in, uint32_t src2_data_in, uint32_t src3_in, uint32_t src3_success_data_in, mips_cpu_h cpu_in, mips_mem_h mem_in ) {
		...
		if (type == 1) {
			...
			if(e!=mips_Success){
				fprintf(stderr, "mips_cpu_set_register : failed with error 0x%x.\n", e);
				exit(1);
			}
			...
			if(e!=mips_Success){
				fprintf(stderr, "mips_cpu_get_pc : failed with error 0x%x.\n", e);
				exit(1);
			}
			...
			if(e!=mips_Success){
				fprintf(stderr, "mips_mem_write : failed with error 0x%x.\n", e);
				exit(1);
			}
		} else if (type == 2) {
			src1 = src1_in;
...
void basic_inst_test(test_instr instr, mips_cpu_h cpu_in, mips_error test_error_in) {
	...
	uint32_t got;
	uint32_t pc_new;
	mips_error test_error = mips_Success;
	int passed;
	...
	if(e!=mips_Success){
		fprintf(stderr, "mips_cpu_get_register : failed with error 0x%x.\n", e);
		exit(1);
	}
	...
	if (test_error != mips_Success) {
		if (test_error == test_error_in) {
			if (instr.pc_aft == pc_new) {
				e = mips_cpu_reset(cpu_in);
				if (e != mips_Success) {
					fprintf(stderr, "mips_cpu_reset : failed with error 0x%x.\n", test_error);
					exit(1);
				}
				passed = 1;
			}
			...
			fprintf(stderr, "mips_cpu_step : failed with error 0x%x.\n",test_error);
			exit(1);
		}
    ...
    stringstream ss;
	ss << instr.name << instr.name << "failed" << endl;
...
void sw_test(test_instr instr, mips_cpu_h cpu_in, mips_mem_h mem_in, mips_error test_error_in) {
	...
	if(e!=mips_Success){
		fprintf(stderr, "mips_mem_read : failed with error 0x%x.\n", e);
		exit(1);
	}
...
int main()
{
    ...
    if(e != mips_Success){
        fprintf(stderr, "mips_cpu_set_debug_level failed with error 0x%x.\n", e);
        exit(1);
    }
	...
	test_instr lw2(0, "lw", 2, 0x4, 0, 0x02, 0, 0x12345678, cpu, mem);
